...
router.use(authorize(...CAN_PLANIF));
router.use(requireModule("PLANIF_INTEL"));
// ─────────────────────────────────────────────────────────────
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Erreur analyse dashboard", error: err.message });
    }
  },
);
...
// ─────────────────────────────────────────────────────────────// Remplacer uniquement ce bloc :
router.get(
  "/suggestion-diapason/:clrId",
  authenticate,
  authorize(...CAN_PLANIF),
  async (req, res) => {
    try {
      const result = await getSuggestionDiapason(
        ...
        req.user.companyId,
        req.query.orderId ? parseInt(req.query.orderId) : null, // ← NOUVEAU
      );
      res.json(result);
    } catch (err) {
      console.error(err);
      res
        .status(err.message === "CLR introuvable" ? 404 : 500)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Erreur analyse alertes", error: err.message });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!clrId || !quantite || !diapason) {
        return res
          .status(400)
          .json({ message: "clrId, quantite et diapason sont requis" });
      }
      if (!["D1", "D2", "D3", "D4", "D5"].includes(diapason)) {
        return res
          .status(400)
          .json({ message: "diapason doit être D1, D2, D3, D4 ou D5" });
      }
      if (quantite <= 0) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(err);
      res
        .status(err.message === "Session introuvable" ? 404 : 500)
        .json({ message: err.message });
    }
  },
);
